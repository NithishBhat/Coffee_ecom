...
Indian coffee bean e-commerce store built with the MERN stack. Customers browse products, add to cart, checkout with Razorpay payments, and track orders. Admins manage products, orders, reviews, and view sales analytics. Email notifications are sent on order events via SMTP.
...
└── server/
    ...
    │   ├── Order.js                   # orderId (random ORD-XXXXXX), customer, items, amounts, Razorpay IDs, payment/fulfillment status
...
4. On payment success, `POST /api/orders/verify` — verifies HMAC signature, updates order to `paid`, decrements stock, checks low stock alerts, sends confirmation email
5. Phone saved to localStorage, redirects to `/order/:orderId`
6. OrderConfirmation page loads order, clears cart, shows status tracker

### Order Tracking
- **Phone only**: `GET /api/orders/track-by-phone` — returns summary list (orderId, date, total, status only)
- **Phone + Order ID**: `GET /api/orders/track` — returns full order details
...
| GET | `/api/admin/stats` | Analytics: period stats, daily chart, top products, avg order value |
